...
class QuizGenerator(object):
    ...
    def __init__(self):
        ...
        with open(
            os.path.join(default_path, "prompt_tools", "prompt_start.txt"), "r"
        ) as file:
            self.prompt_start_text = file.read()

        with open(
            os.path.join(default_path, "prompt_tools", "prompt_end.txt"), "r"
        ) as file:
            self.prompt_end_text = file.read()

        with open(
            os.path.join(default_path, "prompt_tools", "prompt_json_schema.txt"), "r"
        ) as file:
            self.prompt_josn_schema_text = file.read()

        googleGenai.configure(api_key=os.environ["GOOGLE_API_KEY"])
        self.model = googleGenai.GenerativeModel("gemini-2.5-pro-exp-03-25")

    def generate_quiz(
        self, quiz_description: str, files: Optional[list] = None
    ) -> Dict[str, Any]:
        """
        ...
        if files:
            for file in files:
                ...
                temp_path = os.path.join(
                    temp_dir,
                    f"temp_{os.urandom(8).hex()}{os.path.splitext(file.filename)[1]}",
                )
